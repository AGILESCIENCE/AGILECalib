...
; Calling:
; - readedp_H0025.pro, interp_edp_H0025.pro, initheads.pro
; Output:
; AG_GRID_G0017_S<filter><eventtype>_H0025.edp.gz
; ---------------------------------------
; Authors:
; A. Chen (INAF/IASF Mi)
; ---------------------------------------
...
template = edp_path+'/edp_template_H.edp'
fileid = 'H0025'
...
for ifilter = 0, n_elements(filters)-1 do $
   for istatus = 0, n_elements(status)-1 do begin
        ...
        for itheta = 0, nthetas - 1 do for iphi = 0, nphis - 1 do begin
            readedp_H0025, filters[ifilter], thetas[itheta], phis[iphi], resp, $
              dir = rsp_path+'/'
            allresps[*, *, *, itheta, iphi] = resp
        endfor
        interp_edp_H0025, table, allresps, labels, thetas, phis, istatus
