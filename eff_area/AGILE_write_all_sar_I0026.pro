...
; Output:
; AG_GRID_G0017_S${filter}${eventtype}_I0026.sar.gz
; ---------------------------------------
; Authors:
; A. Chen (INAF/IASF Mi)
; ---------------------------------------
; Modification history:
; 06/10/2023: copied from AGILE_write_all_sar_I0023.pro (V. Fioretti)
...
readaeff_I0026, ae_path, filters[0], thetas[0], phis[0], energies, areas
...
for ifilter = 0, n_elements(filters)-1 do $
   for istatus = 0, n_elements(status)-1 do begin
        ...
        for itheta = 0, nthetas - 1 do for iphi = 0, nphis - 1 do begin
            readaeff_I0026, ae_path, filters[ifilter], thetas[itheta], phis[iphi], energies, areas
            allareas[*, itheta, iphi] = areas
        endfor
        interp_sar_I0026, table, allareas, labels, energies, thetas, phis, istatus, factor = factor[*,ifilter]
